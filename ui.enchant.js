...
enchant.ui.Pad = enchant.Class.create(enchant.Sprite, {
  ...
  initialize: function () {
    ...
    const image = core.assets['pad.png']
...
enchant.ui.APad = enchant.Class.create(enchant.Group, {
  ...
  initialize: function (mode) {
    ...
    const insideImage = new enchant.Surface(w / 2, h / 2)
  ...
  _dispatchPadEvent: function (type) {
    const e = new enchant.Event(type)
    e.vx = this.vx
    e.vy = this.vy
    e.rad = this.rad
    e.dist = this.dist
...
enchant.ui.Button = enchant.Class.create(enchant.Entity, {
  ...
  _applyTheme: function (theme) {
    const style = this._style
    const css = enchant.ui.Button.theme2css(theme)
    for (const i in css) {
  ...
  domRender: function () {
    const element = this._domManager.element
...
enchant.ui.Button.theme2css = function (theme) {
  const prefix = '-' + enchant.ENV.VENDOR_PREFIX.toLowerCase() + '-'
  const obj = {}
  const bg = theme.background
  const bd = theme.border
  const ts = theme.textShadow
  const bs = theme.boxShadow
  if (prefix === '-ms-') {
...
enchant.ui.MutableText = enchant.Class.create(enchant.Sprite, {
  ...
  setText: function (txt) {
    let i, x, y, wNum, charCode, charPos
    this._text = txt
    let newWidth
  ...
  initialize: function (x, y) {
    ...
    this.addEventListener('enterframe', function () {
      if (this.easing === 0) {
        ...
        const dist = this._score - this._current
        if (dist > 0) {
