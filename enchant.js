  ...
  if (typeof Function.prototype.bind !== 'function') {
    Function.prototype.bind = function (thisObject) {
      ...
      const bound = function () {
        const a = args.concat(Array.prototype.slice.call(arguments))
        return func.apply(
 ...
 */
  enchant.Class.create = function (superclass, definition) {
    ...
    const tree = this.getInheritanceTree(superclass)
    ...
    enchant.Core = enchant.Class.create(enchant.EventTarget, {
      ...
      initialize: function (width, height) {
        ...
        if (!stage) {
          ...
          const bounding = stage.getBoundingClientRect()
        ...
        for (const prop in enchant.ENV.KEY_BIND_TABLE) {
        ...
        if (initial) {
          ...
          if (enchant.ENV.TOUCH_ENABLED) {
            stage.addEventListener('touchstart', function (e) {
              const core = enchant.Core.instance
              const evt = new enchant.Event(enchant.Event.TOUCH_START)
            ...
            stage.addEventListener('touchmove', function (e) {
              const core = enchant.Core.instance
              const evt = new enchant.Event(enchant.Event.TOUCH_MOVE)
              const touches = e.changedTouches
              let touch, target
              for (let i = 0, l = touches.length; i < l; i++) {
                touch = touches[i]
            ...
            stage.addEventListener('touchend', function (e) {
              const core = enchant.Core.instance
              const evt = new enchant.Event(enchant.Event.TOUCH_END)
              const touches = e.changedTouches
              let touch, target
              for (let i = 0, l = touches.length; i < l; i++) {
                touch = touches[i]
          ...
          stage.addEventListener('mousedown', function (e) {
            ...
            const target = core.currentScene._determineEventTarget(evt)
          ...
          stage.addEventListener('mousemove', function (e) {
            ...
            const evt = new enchant.Event(enchant.Event.TOUCH_MOVE)
            evt._initPosition(e.pageX, e.pageY)
            const target = core._touchEventTarget[core._mousedownID]
            if (target) {
              target.dispatchEvent(evt)
            }
          }, false)
      ...
      _dispatchCoreResizeEvent: function () {
        const e = new enchant.Event('coreresize')
        e.width = this._width
      ...
      load: function (src, alias, callback, onerror) {
        ...
        const ext = enchant.Core.findExt(src)

        return enchant.Deferred.next(function () {
          const d = new enchant.Deferred()
            ...
            req.onreadystatechange = function () {
              if (req.readyState === 4) {
                if (req.status !== 200 && req.status !== 0) {
                  ...
                  const e = new enchant.Event('error')
      ...
      start: function (deferred) {
        ...
        if (!this._activated) {
          ...
          if (enchant.ENV.BROWSER === 'mobilesafari' &&
            ...
            const d = new enchant.Deferred()
            const scene = this._createTouchToStartScene()
            scene.addEventListener(enchant.Event.TOUCH_START, function waitTouch () {
              ...
              const a = new enchant.WebAudioSound()
        ...
        const ret = this._requestPreload()
          .next(function () {
      ...
      _requestPreload: function () {
        ...
        const loadFunc = function () {
          const e = new enchant.Event('progress')
          e.loaded = ++loaded
          e.total = len
      ...
      _createTouchToStartScene: function () {
        const label = new enchant.Label('Touch to Start')
        const size = Math.round(core.width / 10)
        const scene = new enchant.Scene()
      ...
      keyunbind: function (key) {
        const button = this._keybind[key]
  ...
  enchant.InputManager = enchant.Class.create(enchant.EventTarget, {
    ...
    initialize: function (valueStore, source) {
      ...
      this._stateHandler = function (e) {
        const id = e.source.identifier
        const name = this._binds[id]
    ...
    addBroadcastTarget: function (eventTarget) {
      const i = this.broadcastTarget.indexOf(eventTarget)
      if (i === -1) {
 ...
 */
  enchant.InputSource = enchant.Class.create(enchant.EventTarget, {
    ...
    notifyStateChange: function (data) {
      const e = new enchant.Event(enchant.Event.INPUT_STATE_CHANGED)
      e.data = data
      e.source = this
      this.dispatchEvent(e)
    }
 ...
 */
  enchant.BinaryInputManager = enchant.Class.create(enchant.InputManager, {
    ...
    unbind: function (binaryInputSource) {
      const name = this._binds[binaryInputSource.identifier]
    ...
    _down: function (name) {
      ...
      const downEvent = new enchant.Event(name + this.activeEventNameSuffix)
    ...
    _up: function (name) {
      ...
      const upEvent = new enchant.Event(name + this.inactiveEventNameSuffix)
  ...
  enchant.KeyboardInputManager = enchant.Class.create(enchant.BinaryInputManager, {
    ...
    _attachDOMEvent: function (domElement, eventType, state) {
      domElement.addEventListener(eventType, function (e) {
        ...
        const source = enchant.KeyboardInputSource._instances[code]
        if (source) {
 ...
 */
  enchant.Node = enchant.Class.create(enchant.EventTarget, {
    ...
    _updateCoordinate: function () {
      ...
      const matrix = enchant.Matrix.instance
  ...
  const _intersectBetweenClassAndInstance = function (Class, instance) {
    const ret = []
    let c
  ...
  const _intersectBetweenClassAndClass = function (Class1, Class2) {
    const ret = []
    let c1, c2
  ...
  const _intersectStrictBetweenClassAndInstance = function (Class, instance) {
    const ret = []
    let c
    for (let i = 0, l = Class.collection.length; i < l; i++) {
      c = Class.collection[i]
  ...
  const _intersectStrictBetweenClassAndClass = function (Class1, Class2) {
    const ret = []
    let c1, c2
    for (let i = 0, l = Class1.collection.length; i < l; i++) {
      c1 = Class1.collection[i]
  ...
  const _staticIntersect = function (other) {
  ...
  const _staticIntersectStrict = function (other) {
    if (other instanceof enchant.Entity) {
  ...
  const _nodePrototypeClearEventListener = enchant.Node.prototype.clearEventListener
 ...
 */
  enchant.Entity = enchant.Class.create(enchant.Node, {
    ...
    _intersectStrictOne: function (other) {
      ...
      const rect1 = this.getOrientedBoundingRect()
      const rect2 = other.getOrientedBoundingRect()
      const lt1 = rect1.leftTop; const rt1 = rect1.rightTop
      const lb1 = rect1.leftBottom; const rb1 = rect1.rightBottom
      const lt2 = rect2.leftTop; const rt2 = rect2.rightTop
      const lb2 = rect2.leftBottom; const rb2 = rect2.rightBottom
      const ltx1 = lt1[0]; const lty1 = lt1[1]; const rtx1 = rt1[0]; const rty1 = rt1[1]
      const lbx1 = lb1[0]; const lby1 = lb1[1]; const rbx1 = rb1[0]; const rby1 = rb1[1]
      const ltx2 = lt2[0]; const lty2 = lt2[1]; const rtx2 = rt2[0]; const rty2 = rt2[1]
      const lbx2 = lb2[0]; const lby2 = lb2[1]; const rbx2 = rb2[0]; const rby2 = rb2[1]
      const t1 = [rtx1 - ltx1, rty1 - lty1]
      const r1 = [rbx1 - rtx1, rby1 - rty1]
      const b1 = [lbx1 - rbx1, lby1 - rby1]
      const l1 = [ltx1 - lbx1, lty1 - lby1]
      const t2 = [rtx2 - ltx2, rty2 - lty2]
      const r2 = [rbx2 - rtx2, rby2 - rty2]
      const b2 = [lbx2 - rbx2, lby2 - rby2]
      const l2 = [ltx2 - lbx2, lty2 - lby2]
      const cx1 = (ltx1 + rtx1 + lbx1 + rbx1) >> 2
      const cy1 = (lty1 + rty1 + lby1 + rby1) >> 2
      const cx2 = (ltx2 + rtx2 + lbx2 + rbx2) >> 2
      const cy2 = (lty2 + rty2 + lby2 + rby2) >> 2
      let i; let j; let poss1; let poss2; let dirs1; let dirs2; let pos1; let pos2; let dir1; let dir2
      let px1; let py1; let px2; let py2; let dx1; let dy1; let dx2; let dy2; let vx; let vy; let c; let c1; let c2
    ...
    _removeSelfFromCollection: function () {
      ...
      const Constructor = this.getConstructor()
      Constructor._collectionTarget.forEach(function (C) {
        const i = C.collection.indexOf(this)
        if (i !== -1) {
    ...
    getBoundingRect: function () {
      ...
      const m11w = mat[0] * w; const m12w = mat[1] * w
      const m21h = mat[2] * h; const m22h = mat[3] * h
      const mdx = mat[4]; const mdy = mat[5]
      const xw = [mdx, m11w + mdx, m21h + mdx, m11w + m21h + mdx].sort(function (a, b) { return a - b })
      const yh = [mdy, m12w + mdy, m22h + mdy, m12w + m22h + mdy].sort(function (a, b) { return a - b })
  ...
  const _collectizeConstructor = function (Constructor) {
    if (Constructor._collective) {
      return
    }
    const rel = enchant.Class.getInheritanceTree(Constructor)
    const i = rel.indexOf(enchant.Entity)
    if (i !== -1) {
 ...
 */
  enchant.Sprite = enchant.Class.create(enchant.Entity, {
    ...
    cvsRender: function (ctx) {
      ...
      let iw; let ih; let elem; let sx; let sy; let sw; let sh
      if (image && w !== 0 && h !== 0) {
        iw = image.width
        ih = image.height
        if (iw < w || ih < h) {
  ...
  enchant.Label = enchant.Class.create(enchant.Entity, {
    ...
    text: {
      ...
      set: function (text) {
        ...
        this.updateBoundArea()
        for (let i = 0, l = this._splitText.length; i < l; i++) {
 ...
 */
  enchant.Map = enchant.Class.create(enchant.Entity, {
    ...
    initialize: function (tileWidth, tileHeight) {
      ...
      const surface = new enchant.Surface(core.width, core.height)
    ...
    loadData: function (data) {
      ...
      for (let i = 0, len = this._data.length; i < len; i++) {
        ...
        for (let y = 0, l = data.length; y < l; y++) {
          for (let x = 0, ll = data[y].length; x < ll; x++) {
            if (data[y][x] >= 0) {
              c++
            }
          }
    ...
    checkTile: function (x, y) {
      ...
      const tileWidth = this._tileWidth || width
      const tileHeight = this._tileHeight || height
      x = x / tileWidth | 0
      y = y / tileHeight | 0
      ...
      if (this.collisionData != null) {
        ...
        for (let i = 0, len = this._data.length; i < len; i++) {
          ...
          if (data[y] != null && (n = data[y][x]) != null &&
                    n >= 0 && n < (width / tileWidth | 0) * (height / tileHeight | 0)) {
            return true
          }
        }
        return false
    ...
    image: {
      ...
      set: function (image) {
        ...
        this._image = image
        if (enchant.ENV.RETINA_DISPLAY && core.scale === 2) {
          const img = new enchant.Surface(image.width * 2, image.height * 2)
          const tileWidth = this._tileWidth || image.width
          const tileHeight = this._tileHeight || image.height
    ...
    updateBuffer: function () {
      if (this._visible === undefined || this._visible) {
        ...
        if (this._dirty || this._previousOffsetX == null) {
          ...
          if (this._tight) {
            const x = -this._offsetX
            const y = -this._offsetY
            const px = -this._previousOffsetX
            const py = -this._previousOffsetY
            const w1 = x - px + core.width
            const w2 = px - x + core.width
            const h1 = y - py + core.height
            const h2 = py - y + core.height
            if (w1 > this._tileWidth && w2 > this._tileWidth &&
                            h1 > this._tileHeight && h2 > this._tileHeight) {
              let sx, sy, dx, dy, sw, sh
              if (w1 < w2) {
                sx = 0
                dx = px - x
                sw = w1
              ...
              let context = core._buffer.getContext('2d')
              if (this._doubledImage) {
    ...
    cvsRender: function (ctx) {
      if (this.width !== 0 && this.height !== 0) {
        ...
        const cvs = this._context.canvas
 ...
 */
  enchant.Group = enchant.Class.create(enchant.Node, {
    ...
    addChild: function (node) {
      ...
      const childAdded = new enchant.Event('childadded')
      ...
      if (this.scene) {
        ...
        const addedToScene = new enchant.Event('addedtoscene')
    ...
    removeChild: function (node) {
      ...
      if ((i = this.childNodes.indexOf(node)) !== -1) {
        ...
        const childRemoved = new enchant.Event('childremoved')
        childRemoved.node = node
        this.dispatchEvent(childRemoved)
        node.dispatchEvent(new enchant.Event('removed'))
        if (this.scene) {
          node.scene = null
          const removedFromScene = new enchant.Event('removedfromscene')
  ...
  enchant.Matrix = enchant.Class.create({
    ...
    makeTransformMatrix: function (node, dest) {
      ...
      const rotation = node._rotation || 0
      const scaleX = (typeof node._scaleX === 'number') ? node._scaleX : 1
      const scaleY = (typeof node._scaleY === 'number') ? node._scaleY : 1
      const theta = rotation * Math.PI / 180
      const tmpcos = Math.cos(theta)
      const tmpsin = Math.sin(theta)
      const w = (typeof node._originX === 'number') ? node._originX : width / 2
      const h = (typeof node._originY === 'number') ? node._originY : height / 2
      const a = scaleX * tmpcos
      const b = scaleX * tmpsin
      const c = scaleY * tmpsin
      const d = scaleY * tmpcos
      dest[0] = a
      dest[1] = b
      dest[2] = -c
      dest[3] = d
    ...
    multiply: function (m1, m2, dest) {
      const a11 = m1[0]; const a21 = m1[2]; const adx = m1[4]
      const a12 = m1[1]; const a22 = m1[3]; const ady = m1[5]
      const b11 = m2[0]; const b21 = m2[2]; const bdx = m2[4]
      const b12 = m2[1]; const b22 = m2[3]; const bdy = m2[5]
    ...
    multiplyVec: function (mat, vec, dest) {
      ...
      const m11 = mat[0]; const m21 = mat[2]; const mdx = mat[4]
      const m12 = mat[1]; const m22 = mat[3]; const mdy = mat[5]
  ...
  enchant.DetectColorManager = enchant.Class.create({
    ...
    initialize: function (reso, max) {
      ...
      for (let i = 1, l = this.capacity; i < l; i++) {
    ...
    detachDetectColor: function (sprite) {
      const i = this.reference.indexOf(sprite)
      if (i !== -1) {
        this.reference[i] = null
      }
    },
    _getColor: function (n) {
      const C = this.colorResolution
      const d = C / this.max
      return [
  ...
  enchant.DomManager = enchant.Class.create({
    ...
    getNextManager: function (manager) {
      const i = this.targetNode.parentNode.childNodes.indexOf(manager.targetNode)
    ...
    addManager: function (childManager, nextManager) {
      ...
      const element = childManager.getDomElement()
    ...
    render: function (inheritMat) {
      ...
      const ox = (typeof node._originX === 'number') ? node._originX : node.width / 2 || 0
      const oy = (typeof node._originY === 'number') ? node._originY : node.height / 2 || 0
      const vec = [ox, oy]
  ...
  enchant.DomlessManager = enchant.Class.create({
    ...
    _register: function (element, nextElement) {
      const i = this._domRef.indexOf(nextElement)
      if (element instanceof Array) {
        if (i === -1) {
    ...
    getDomElementAsNext: function () {
      if (this._domRef.length) {
        ...
        const nextManager = this.getNextManager(this)
        if (nextManager) {
    ...
    addManager: function (childManager, nextManager) {
      const parentNode = this.targetNode.parentNode
      if (parentNode) {
        if (nextManager === null) {
      ...
      const nextElement = nextManager ? nextManager.getDomElementAsNext() : null
      this._register(childManager.getDomElement(), nextElement)
      this.setLayer(this.layer)
    },
    removeManager: function (childManager) {
      let dom
      const i = this._domRef.indexOf(childManager.element)
      if (i !== -1) {
        dom = this._domRef[i]
  ...
  enchant.DomLayer = enchant.Class.create(enchant.Group, {
    ...
    initialize: function () {
      ...
      var __onchildadded = function (e) {
        ...
        const next = e.next
        const self = e.target
        const nextManager = next ? next._domManager : null
        ...
        const render = new enchant.Event(enchant.Event.RENDER)
        child._dirty = true
    ...
    _determineEventTarget: function () {
      const target = this._touchEventTarget
      this._touchEventTarget = null
 ...
 */
  enchant.CanvasLayer = enchant.Class.create(enchant.Group, {
    ...
    _getEntityByPosition: function (x, y) {
      ...
      const extra = enchant.ENV.COLOR_DETECTION_LEVEL - 1
      const rgba = ctx.getImageData(x - extra, y - extra, 1 + extra * 2, 1 + extra * 2).data
  ...
  enchant.Scene = enchant.Class.create(enchant.Group, {
    ...
    initialize: function () {
      ...
      this._dispatchExitframe = function () {
        let layer
        for (const prop in that._layers) {
    ...
    x: {
      ...
      set: function (x) {
        this._x = x
        for (const type in this._layers) {
    ...
    addLayer: function (type, i) {
      ...
      const layer = new enchant[type + 'Layer']()
      if (core.currentScene === this) {
        layer._startRendering()
      }
      this._layers[type] = layer
      const element = layer._element
      if (typeof i === 'number') {
        const nextSibling = this._element.childNodes[i]
        if (nextSibling) {
 ...
 */
  enchant.LoadingScene = enchant.Class.create(enchant.Scene, {
    ...
    initialize: function () {
      ...
      const barWidth = this.width * 0.4 | 0
      const barHeight = this.width * 0.05 | 0
      const border = barWidth * 0.03 | 0
      const bar = new enchant.Sprite(barWidth, barHeight)
      bar.disableCollection()
      bar.x = (this.width - barWidth) / 2
      bar.y = (this.height - barHeight) / 2
      const image = new enchant.Surface(barWidth, barHeight)
      ...
      bar.image = image
      let progress = 0; let _progress = 0
      this.addEventListener('progress', function (e) {
 ...
 */
  enchant.CanvasScene = enchant.Class.create(enchant.Scene, {
    ...
    _determineEventTarget: function (e) {
      let target = this._layers.Canvas._determineEventTarget(e)
      if (!target) {
        target = this
      }
      return target
 ...
 */
  enchant.DOMScene = enchant.Class.create(enchant.Scene, {
    ...
    _determineEventTarget: function (e) {
      let target = this._layers.Dom._determineEventTarget(e)
      if (!target) {
        target = this
      }
      return target
 ...
 */
  enchant.Surface = enchant.Class.create(enchant.EventTarget, {
    ...
    initialize: function (width, height) {
      ...
      const id = 'enchant-surface' + core._surfaceID++
     ...
     */
    clone: function () {
      const clone = new enchant.Surface(this.width, this.height)
      clone.draw(this)
      return clone
    },
    /**
 ...
 */
  ...
  if (window.Deferred) {
    ...
    enchant.Deferred.next = function (func) {
      const q = new enchant.Deferred().next(func)
    ...
    enchant.Deferred.parallel = function (arg) {
      ...
      q._id = setTimeout(function () { q.call() }, 0)
      let progress = 0
      const ret = (arg instanceof Array) ? [] : {}
      const p = new enchant.Deferred()
 ...
 */
  enchant.DOMSound.load = function (src, type, callback, onerror) {
    ...
    const sound = Object.create(enchant.DOMSound.prototype)
    ...
    if (!enchant.ENV.SOUND_ENABLED_ON_MOBILE_SAFARI &&
      ...
      if (!enchant.ENV.USE_FLASH_SOUND && audio.canPlayType(type)) {
        ...
        const id = 'enchant-audio' + enchant.Core.instance._soundID++
 ...
 */
  enchant.WebAudioSound = enchant.Class.create(enchant.EventTarget, {
    ...
    play: function (dup) {
      ...
      const offset = this._currentTime
     ...
     */
    clone: function () {
      const sound = new enchant.WebAudioSound()
 ...
 */
  enchant.WebAudioSound.load = function (src, type, callback, onerror) {
    const canPlay = (new Audio()).canPlayType(type)
    const sound = new enchant.WebAudioSound()
    callback = callback || function () {}
    onerror = onerror || function () {}
 ...
 */
  enchant.ActionEventTarget = enchant.Class.create(enchant.EventTarget, {
    ...
    dispatchEvent: function (e) {
      const target = this.node ? this.node : this
 ...
 */
 ...
     * You can choose time based and frame based(default) animation.
    ...
    next: function (remainingTime) {
      let e; const action = this.queue.shift()
      ...
      if (remainingTime > 0 || (this.queue[0] && this.queue[0].time === 0)) {
        const event = new enchant.Event('actiontick')
    ...
    tick: function (elapsed) {
      if (this.queue.length > 0) {
        ...
        const e = new enchant.Event('actiontick')
        e.timeline = this
        e.elapsed = elapsed
        action.dispatchEvent(e)
      }
    ...
    add: function (action) {
      ...
      let e = new enchant.Event('addedtotimeline')
      e.timeline = this
      action.dispatchEvent(e)
     ...
     */
    clear: function () {
      const e = new enchant.Event('removedfromtimeline')
      e.timeline = this
    ...
    skip: function (frames) {
      const event = new enchant.Event('enterframe')
    ...
    and: function () {
      ...
      if (last instanceof enchant.ParallelAction) {
        ...
        const parallel = new enchant.ParallelAction()
...
     * You can specify a listener for these events to perform specific events when they occur.
    ...
    initialize: function (param) {
      ...
      this.addEventListener(enchant.Event.ACTION_TICK, function (evt) {
        const remaining = action.time - (action.frame + evt.elapsed)
  ...
  enchant.ParallelAction = enchant.Class.create(enchant.Action, {
    ...
    initialize: function (param) {
      ...
      this.addEventListener(enchant.Event.ACTION_START, function (evt) {
        for (let i = 0, len = that.actions.length; i < len; i++) {
          that.actions[i].dispatchEvent(evt)
        }
      })

      this.addEventListener(enchant.Event.ACTION_TICK, function (evt) {
        let i; let len; const timeline = {
          next: function (remaining) {
            const action = that.actions[i]
            that.actions.splice(i--, 1)
            len = that.actions.length
            that.endedActions.push(action)

            let e = new enchant.Event('actionend')
            e.timeline = this
            action.dispatchEvent(e)
  ...
  enchant.Tween = enchant.Class.create(enchant.Action, {
    ...
    initialize: function (params) {
      ...
      this.addEventListener(enchant.Event.ACTION_START, function () {
        // excepted property
        const excepted = ['frame', 'time', 'callback', 'onactiontick', 'onactionstart', 'onactionend']
      ...
      this.addEventListener(enchant.Event.ACTION_TICK, function (evt) {
        ...
        const ratio = tween.time === 0 ? 1 : tween.easing(Math.min(tween.time, tween.frame + evt.elapsed), 0, 1, tween.time) - tween.easing(tween.frame, 0, 1, tween.time)
